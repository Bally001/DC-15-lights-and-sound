...
const int muzzleTime = 300;
const int rumbleTime = 600;
...
/*
  if (millis() - fireTime >= muzzleTime)  {
    digitalWrite(muzzleLed, LOW);
  }

  if (millis() - fireTime >= rumbleTime)  {
...
/*
  ...
  digitalWrite(muzzleLed, HIGH);
    */
  pulseImmediate(muzzleLed, 100, HIGH);
  pulseImmediate(rumble, 300, HIGH);
